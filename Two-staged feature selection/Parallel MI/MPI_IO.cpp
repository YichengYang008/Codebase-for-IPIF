...
void MPI_Out_raw(const int nrow, const int ncol, MPI_File fh, double** d_matrix) {
	//Description==============================================
	//  Write a matrix (double values)
	//  to the target Binary file from master processor
	//  IN:   double** d_matrix[nrow][ncol]
	//  IN:   numWorkPerProc is particularly read for the last processor
	//
	//Note that this function is especially for writing daty 
	//column-wisely to local storage from master processor
	//==========================================================
...
void MPI_Out_id(const int id_size, MPI_File fh, std::vector<int> selected_Predictor) {
	//Description==============================================
	//  Write a matrix (double values)
	//  to the target Binary file from master processor
	//  IN:   double** d_matrix[nrow][ncol]
	...
	if (mynode == 0) {
		int*   i_temp = new int[id_size];// buffer

		for (int i = 0; i < id_size; i++) {
			i_temp[i] = selected_Predictor[i];
		}
		...
		success = MPI_File_write_at(fh, disp, i_temp, id_size, MPI_INT, &status);
		if (success != MPI_SUCCESS) cout << "MPI I/O fail to write the file!" << endl;
	...
	//==========================================================
{
	...
	for (int i = 0; i < ncol; i++) {
		disp = (unsigned long long)i_row_target * sizeof(double) + i*nrow * sizeof(double);
...
void MPI_In_daty_column(const int nrow, const int ncol, const int i_col_target,
	MPI_File fh, double* d_read_out)
	//Description==============================================
	//  Read one column of raw daty "matrix[nrow][ncol]" (double values)
	//  from the Binary file
	//  return in the vector "d_read_out[nrow]"
	//
	//OUT  : double* d_read_out[ncol]
	...
	//==========================================================
{
	...
	for (int j = 0; j < (totalnodes - 1); j++) { // exclude processor 0
		...
		if (j != (totalnodes - 2)) disp = (unsigned long long)sizeof(double)*numWorkPerProc*i_row_target + sizeof(double)*nrow*numWorkPerProc*j; // offset of starting reading
		if (j == (totalnodes - 2)) disp = (unsigned long long)sizeof(double)*nrow*numWorkPerProc*(totalnodes - 2) + i_row_target*numWorkLocalLast * sizeof(double); // offset of starting reading
...
void MPI_Out_datz_columnwise(const int nrow, const int ncol, int numWorkPerProc,
	MPI_File fh, double** d_matrix) {
	//Description==============================================
	//  Write a matrix (double values)
	//  to the target Binary file from multiple processors concurrently
	...
	if (mynode != 0) {
		...
		for (int j = 0; j < ncol; j++) {
			for (int i = 0; i < nrow; i++) {
				d_temp[counter] = d_matrix[i][j]; // copy elements in matrix to buffer column-wisely
				counter++;
			}
		}
...
void MPI_Out_datz_rowise(const int nrow, const int ncol, const int ncol_total, int numWorkPerProc,
	...
	if (mynode != 0) {
		...
		for (int i = 0; i < nrow; i++) {
			...
			disp = 0;
			disp = (unsigned long long)(mynode - 1)*numWorkPerProc * sizeof(double) + i*ncol_total * sizeof(double);
