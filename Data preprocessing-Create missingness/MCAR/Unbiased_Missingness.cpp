...
int main(int argc, char *argv[]) {
	...
	cout<<"Start missing at random!!!"<<endl;

	const int nrow = 106574;
	const int ncol = 518;
	double rho = 0.5;
	...
	if (mynode != 0) {
		for (int i = 0; i < nrow_sample; i++) {
			if (final_row_indicator[i] == 0) {
				if (mynode == 2) { cout << "CAUTION! The " << i << "th row of datr1 is all zero!" << endl; }
				for (int l = 0; l < L_temp; l++) {
					datr1[i][l] = 1;
				}
			}
