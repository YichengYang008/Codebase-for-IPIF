...
void AGMAT_Extension_Bigp_cpp(double** mox, const int nrow_mox,
	...
	std::string cn[], const int nrow, const int i_collapsing, int** codes,
	std::vector<std::string> &rst_final, ofstream& TestOut)
	//Description=========================================
	...
	//====================================================
{
	...
	//if (mynode == 0) cout << "AGMAT 1 where nr1 is " << nr1 << endl;
	...
	for (int i = 0; i<nr1; i++)
	{
		...
		if (i_sum_x > 0)
		{
			...
			//if (mynode == 0) cout<<"AGMAT nl is "<<nl<<" at mox "<<i<<endl;
			//-------
			...
			for (int k = 0; k < i_collapsing; k++) {
				if (codes[i][k] != 0) {
					...
					//if(mynode==0) cout<<"AGMAT v_mxl["<<i<<"]["<<k<<"]: "<< codes[i][k] <<endl;
				}
			}
			...
			*/
			...
			if (i_size_loc_srst_nl > 0)
			{
				...
				//if (mynode == 0) cout << " nl is " << nl << ", i_size_v_cn_z_i is " << i_size_v_cn_z_i << ", i_size_loc_srst_nl is " << i_size_loc_srst_nl << ", i_size_zid at iteration " << i << " is " << i_size_zid << endl;
