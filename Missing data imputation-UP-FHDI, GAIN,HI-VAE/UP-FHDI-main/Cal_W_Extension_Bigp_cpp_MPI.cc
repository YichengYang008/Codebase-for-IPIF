...
void Cal_W_Extension_Bigp_cpp_MPI(double** mox, const int nrow_mox,
	...
	//====================================================
{
	...
	if (n_row_rst < totalnodes) {
		cout << "Error!!! The size of i_rst_id in Cal_w is too small for parallel" << endl;
		return;
	}

	//if (mynode == 1) {
	//	for (int i = 0; i < n_row_rst; i++) {
	//		cout<<"i_rst_id["<<i<<"]: "<< i_rst_id[i] <<" at i_loop "<< i_loop <<" at node "<<mynode <<endl;
	//	}
	//}
	order_FHDI_Yicheng(i_rst_id, n_row_rst); //returned with the order of rows in ascending magnitude
	//if (mynode == 1) {
	//	cout << "Successful out order_FHDI at i_loop " << i_loop << " at mynode "<<mynode << endl;
	//}
									 //testout
