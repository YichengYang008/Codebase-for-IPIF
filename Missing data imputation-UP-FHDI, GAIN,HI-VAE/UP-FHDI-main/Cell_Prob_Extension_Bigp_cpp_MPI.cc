#include "AGMAT_Extension_Bigp_cpp_MPI.cc"
#include "Cal_W_Extension_Bigp_cpp_MPI.cc"
#include <vector>

void Cell_Prob_Extension_Bigp_cpp_MPI(double** z, const int nrow, const int ncol, const int i_collapsing,
...
//====================================================
{
	...
	*/
	...
	//===================================
	//===================================
	double cell_prob_main_begin = MPI_Wtime();
	...
	for(int i_loop=0; i_loop<n_maximum_iteration; i_loop++)
	{
		...
		//---------------------------------------
		double Cal_w_begin = MPI_Wtime();

		w20.clear();  //re-initialize 
		Cal_W_Extension_Bigp_cpp_MPI(mox, i_count_mox, 
		...
		//testout
		//cout << "Version check 2 at iterations " << i_loop << endl;
		...
		//combine new weights
		//-----------
		double wpct_FHDI_begin = MPI_Wtime();
		...
		if(dif < 1e-6) 
		{
			//cout << "Cell_Prob_Extension_BigP_cpp_MPI.. has successfully finished at iterations " << i_loop << endl;
	...
	//cout<<"Version check Yicheng"<<endl;
	//---------------------------
	//prep return 
