...
void max_occur_union_ultra(std::vector<int> v_table_name, std::vector<int> v_table_counts, int ncol, int size_i, std::vector<int> v_lm, int v_lm_size,
...
//=====================================================

	{
		...
		for (int b1 = 0; b1 < (ncol - v_lm_size); b1++) {
			...
			max_occur_union(v_table_name, v_table_counts, ncol, size_i, v_lm, v_lm_size, b1, i_option_collapsing, v_mxl, correlation_yicheng, correlation_temp2, TestOut);
...
void correlated_variable_union2(const int ncol, const int i_option_collapsing, const int top, int i, int nrow_ol, int* ia_temp,
...

	{
		...
		for (int b1 = 0; b1 < top; b1++) {
			...
			max_occur_union2(v_table_name, v_table_counts, ncol, size_i, v_lm, v_lm_size, i_option_collapsing, top, nrow_ol,  v_mxl, ol_matrix, correlation_temp2, TestOut);
		...
		if (v_mxl.size() < i_option_collapsing) {
			TestOut << "ERROE! The union of top " << top << " ranking matrix is not large enough to get " << i_option_collapsing << " selected variables" << endl;
			exit(0);
		}
		//TestOut << "correlation_temp2 after max_occur at mox i=  " << i << endl;
		//for (int kk2 = 0; kk2 < v_lm_size; kk2++) {
...
void correlated_variable_union_ultra(const int ncol, const int i_option_collapsing, const int top, int i, int* ia_temp, double** correlation_top,
...

	{
		...
		for (int b1 = 0; b1 < top; b1++) {
			...
			max_occur_union_ultra(v_table_name, v_table_counts, ncol, size_i, v_lm, v_lm_size, i_option_collapsing, 
		...
		if (v_mxl.size() < i_option_collapsing) {
			cout << "ERROE! The union of top " << top << " ranking matrix is not large enough to get " << i_option_collapsing << " selected variables" << endl;
			exit(0);
		}
		//TestOut << "correlation_temp2 after max_occur at mox i=  " << i << endl;
		//for (int kk2 = 0; kk2 < v_lm_size; kk2++) {
