...
bool nDAU_ultra_cpp_MPI(const int memory, const int nrow_uox, const int nrow_mox, const int ncol,

	int i_cellmake, MPI_File fh_uox, MPI_File fh_mox, List_FHDI &uox_infor,
...

{
	...
	//cout << "nDAU L_temp is " << L_temp << " at node " << mynode << endl;
	//========================================================
	//Read distributed mox matrix on all slave processors
	//=========================================================
	double nDAU_reading = MPI_Wtime();
	...
	//	cout << "Yicheng Running time of nDAU reading = " << MPI_Wtime() - nDAU_reading << " at node " << mynode << endl;
	//	printf("Yicheng Running time of reading in Zmat = %f seconds\n", MPI_Wtime() - read_begin);
	//}

	//Initialize 
	std::vector<int> v_nD_temp(L_temp); // total number of donors for distributed recipients
	...
	if (recursion_size_last < 10) cout<<"ERROR!!! recursion_size_last is less than 10 in nDAU_ultra!!"<<endl;

	if (mynode == 0) recursion_time = 0;
	//if (mynode == 0 || mynode == 1) cout << "nDAU recursion_time is " << recursion_time << " and recursion_size_last is " << recursion_size_last << " and recursion_size is " << recursion_size << " at node " << mynode << endl;

	//============================================
	//Read recursion_size rows of uox recursively 
	//============================================
	double nDAU_recursion = MPI_Wtime();

	for (int i_recur = 0; i_recur < recursion_time; i_recur++) {

		double nDAU_iteration1 = MPI_Wtime();
		...
		//	cout << "Yicheng Running time of nDAU_iteration1 = " << MPI_Wtime() - nDAU_iteration1 << " at i_recur = " << i_recur << " at node " << mynode << endl;
		//	printf("Yicheng Running time of reading in Zmat = %f seconds\n", MPI_Wtime() - read_begin);
		//}
		...
		//===============================================================================
		double nDAU_iteration2 = MPI_Wtime();

		for (int i = 0; i < L_temp; i++) {
			double nDAU_inner1 = MPI_Wtime();
		    //-----
			...
			//if (mynode == 1 || mynode == 2) {
			//	cout << "Yicheng Running time of nDAU_inner11 = " << MPI_Wtime() - nDAU_inner1 << " at i =" << i << " at i_recur = " << i_recur << " at node " << mynode << endl;
			//}
			...
			//if (mynode == 1 || mynode == 2) {
			//	cout << "Yicheng Running time of nDAU_inner22 = " << MPI_Wtime() - nDAU_inner22 << " at i =" << i << " at i_recur = " << i_recur << " at node " << mynode << endl;
			//}

			//if (mynode == 1 || mynode == 2) {
			//	cout << "Yicheng Running time of nDAU_inner1 = " << MPI_Wtime() - nDAU_inner1 <<" at i ="<<i<< " at i_recur = " << i_recur << " at node " << mynode << endl;
			//}

			double nDAU_inner2 = MPI_Wtime();
			...
			//if (mynode == 1 || mynode == 2) {
			//	cout << "Yicheng Running time of nDAU_inner2 = " << MPI_Wtime() - nDAU_inner2 <<" at i = "<<i<< " at i_recur = " << i_recur << " at node " << mynode << endl;
			//}

		}//end of main loop

		//if (mynode == 1 || mynode == 2) {
		//	cout << "Yicheng Running time of nDAU_iteration2 = " << MPI_Wtime() - nDAU_iteration2 << " at i_recur = " << i_recur << " at node " << mynode << endl;
		//	printf("Yicheng Running time of reading in Zmat = %f seconds\n", MPI_Wtime() - read_begin);
		//}
	...
	//if (mynode == 1 || mynode == 0) {
	//	cout << "Yicheng Running time of nDAU recursion = " << MPI_Wtime() - nDAU_recursion << " at node " << mynode << endl;
	//	printf("Yicheng Running time of reading in Zmat = %f seconds\n", MPI_Wtime() - read_begin);
	//}
	...
	//	cout << "List_nU_temp before KNN at node " << mynode << endl;
	//	for (int kk2 = 0; kk2 < L_temp; kk2++) {
	//		for (int kk3 = 0; kk3 < max_donor_size; kk3++) {
	//			cout << setw(20) << List_nU_temp[kk2][kk3];
	//		}
	//		cout << endl;
	//	}
	//}
	...
	double knn_recursion = MPI_Wtime();
	for (int j_recur = 0; j_recur < recursion_time; j_recur++) {

		double knn_iteration1 = MPI_Wtime();
		...
		//if (mynode == 1 || mynode == 2) {
		//	cout << "Yicheng Running time of knn_iteration1 = " << MPI_Wtime() - knn_iteration1 << " at node " << mynode << endl;
		//	printf("Yicheng Running time of reading in Zmat = %f seconds\n", MPI_Wtime() - read_begin);
		//}
		...
		//===============================================================================
		double knn_iteration2 = MPI_Wtime();
		...
		for (int l = 0; l < L_temp_size; l++) {
			...
			//if (mynode == 1 && l == 0) {
			//	cout<<"YEYE d_min_fdist is "<< d_min_fdist <<" and d_min_temp[0] is "<< d_min_temp[l] <<" and d_second_min_fdist is "<< d_second_min_fdist <<endl;
		...
		//if (mynode == 1 || mynode == 2) {
		//	cout << "Yicheng Running time of knn_iteration2 = " << MPI_Wtime() - knn_iteration2 << " at node " << mynode << endl;
		//	printf("Yicheng Running time of reading in Zmat = %f seconds\n", MPI_Wtime() - read_begin);
		//}

		//if (mynode == 3) {
		//	//for (int j1 = 0; j1 < L_temp_size; j1++) cout << "d_second_min_temp[" << j1 << "]: " << d_second_min_temp[j1] << " at j_recur " << j_recur << endl;
		//	cout << "d_min_temp[0]: " << d_min_temp[0] << " at j_recur " << j_recur << endl;
		//	cout << "d_second_min_temp[0]: " << d_second_min_temp[0] << " at j_recur " << j_recur << endl;
	...
	//if (mynode == 1 || mynode == 0) {
	//	cout << "Yicheng Running time of knn recursion = " << MPI_Wtime() - knn_recursion << " at node " << mynode << endl;
	//	printf("Yicheng Running time of reading in Zmat = %f seconds\n", MPI_Wtime() - read_begin);
	//}
	...
	for (int t = 0; t < L_temp_size; t++) {
		...
		if (v_nD_temp[i_mox] == 1) {
			//if(mynode==1) cout << "CASE0: KNN with v_nD_temp[" << i_mox << "] with 1 donor and max_nf =" << max_nf << endl;
			//-------------
		...
		if (v_nD_temp[i_mox] == 0) {
			...
			if (max_nf >= 2) {
				//if (mynode == 1) cout << "CASE1: KNN with v_nD_temp[" << i_mox << "] with no donor and max_nf = " << max_nf << endl;
				//-------------
				//random number within [1, i_size_xloc]
				//Note: this is ACTUAL location
				//-------------
			...
			if ((max_nf < 2) && (v_floc.size() >= 2)) {
				//if (mynode == 1) cout << "CASE2: KNN with v_nD_temp[" << i_mox << "] with no donor and max_nf = " << max_nf << ", and v_floc size is " << v_floc.size() << endl;
				//-------------
				//random number within [1, i_size_xloc]
				//Note: this is ACTUAL location
				//-------------
			...
			if ((max_nf < 2) && (v_floc.size() == 1)) {
				...
				//if (mynode == 1) cout << "CASE3: KNN with v_nD_temp[" << i_mox << "] and t = "<<t<< " with no donor and max_nf = " << max_nf << ", and v_floc size is " << v_floc.size() << " and v_floc_second size is " << i_size_floc_second << endl;
				...
				//if (mynode == 1) cout<<"CASE3: i_loc_rand_xloc is "<< i_loc_rand_xloc <<endl;
	 ...
	 //if (mynode == 3) {
	 //	cout << "After List_nU at node " << mynode << endl;
	 //	List_nU_temp.print_List_FHDI_yicheng();
	 //}

	//if (mynode == 1 || mynode == 0) {
	//	cout << "Yicheng Running time of knn update = " << MPI_Wtime() - knn_update << " at node " << mynode << endl;
	//	printf("Yicheng Running time of reading in Zmat = %f seconds\n", MPI_Wtime() - read_begin);
	//}

	//------------------------------------------------------------------
	//Send List_nU_temp and v_nD_temp from slave processors to master
	//-----------------------------------------------------------------

	double knn_communication = MPI_Wtime();
	...
	if (mynode != 0) {
		...
		//if (mynode == 3) {
		//	for (int k = 0; k < L_temp; k++) {
		//		cout << "List_nU_temp_size[" << k << "]: " << List_nU_temp_size[k] << " at node " << mynode << " where L_temp is " << L_temp << endl;
		//	}
		//}
	...
	if (mynode == 0) {
		...
		if (List_nU_size_total.size() != nrow_mox) {
			cout<<"ERROR! List_nU_size_total in nDAU is incorrect!"<<endl;
		}
	...
	//	cout<<"List_nU_size_total["<<t<<"]: "<< List_nU_size_total[t]<<" at node "<<mynode<<endl;
	//}
	...
	//if (mynode == 1) {
	//	for (int t = 0; t < i_size_cell_total;t++) {
	//		cout<<"List_nU_cell_total"<<t<<"]: "<< List_nU_cell_total[t]<<endl;
	//	}
	//}
	...
	//if (mynode == 1 || mynode == 0) {
	//	cout << "Yicheng Running time of knn communication = " << MPI_Wtime() - knn_communication << " at node " << mynode << endl;
	//	//printf("Yicheng Running time of reading in Zmat = %f seconds\n", MPI_Wtime() - read_begin);
	//}
