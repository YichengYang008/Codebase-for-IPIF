...
void Validation_cell_make_bigp_knn(const int nrow_uox, const int nrow_mox, const int nrow, const int ncol, const int max_overlap_size, const int i_collapsing,

	MPI_File fh_datz, MPI_File fh_uox, MPI_File fh_mox, int** uox_info_final, int** mox_info_final, int** codes,
	...
	//====================================================
{
	...
	if (mynode == 0) {
		...
		//codes
		//---------
		int** codes_vd = New_iMatrix(nrow_mox, i_collapsing);
		for (int t = 0; t < nrow_mox; t++) {
			for (int l = 0; l < i_collapsing; l++) {
				codes_vd[t][l] = codes[order_mox[t]][l];
			}
		}
		...
		cout << "final codes_vd from node " << mynode << endl;
		for (int kk2 = 0; kk2 < nrow_mox; kk2++) {
			for (int kk3 = 0; kk3 < i_collapsing; kk3++) {
				cout << setw(20) << codes_vd[kk2][kk3];
			}
			cout << endl;
		}
		...
		Del_iMatrix(codes_vd, nrow_mox, i_collapsing);
