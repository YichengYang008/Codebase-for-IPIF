...
bool nDAU_cpp_MPI(double** uox, double** mox, const int nrow_uox, const int nrow_mox, const int ncol,
...

{
	...
	//std::vector<int> List_nU_value_recv_before;
	//std::vector<int> List_nU_value_recv_after;
...

	{
		...
		cout << "ERROR!!! No possible donors with current k in nDAU_cpp bign. Retry with reduced k" << endl;
